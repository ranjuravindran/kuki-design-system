...
export function Avatar({ size = 'md', src, alt = '', className = '', children, ...rest }: AvatarProps) {
  return (
    <span className={`kuki-avatar kuki-avatar--${size} ${className}`} {...rest}>
      {src ? <img src={src} alt={alt} /> : children ?? <PawIcon />}
    </span>
  );
}
...
function PawIcon() {
  return (
    <svg width="50%" height="50%" viewBox="0 -960 960 960" fill="currentColor" aria-hidden>
